#!/usr/bin/env node
/**
 * Seed the house agent recipes (roadmap 3.1 + 3.4 + the builder fleet +
 * the deep research fleet): the four originals, the six builders, and the
 * six deep research plays (Influencer Panel, Comment Mining Sweep,
 * Cross-Channel Sweep, Reddit Rabbit Hole, Video Deep Dive, The Audience
 * Mosaic). Idempotent by slug — safe to re-run. Requires env from
 * .env.local (loaded if present). Mirror of src/lib/mothermode/research/
 * recipes/seed.ts — keep the two in sync.
 */
...
async function main() {
  ...
  for (const seed of SEEDS) {
    const { error } = await db
      .from('mothermode_recipes')
      .upsert({ ...seed, updated_at: new Date().toISOString() }, { onConflict: 'slug' });
    if (error) {
      if (/could not find the table/i.test(error.message)) {
        console.error(
          'The mothermode_recipes table does not exist yet — the recipes migration has not been applied.\n' +
            'Fix (30 seconds):\n' +
            '  1. Open supabase/_bundle_pending.sql (already generated: lineage → embeddings, idempotent)\n' +
            '  2. Paste it into Supabase dashboard → SQL Editor → Run\n' +
            '  3. Re-run this script.\n' +
            'To regenerate the bundle: node scripts/build-migration-bundle.cjs 20261105000000',
        );
        process.exit(1);
      }
    ...
    console.log(`Seeded ${seed.slug} (${seed.steps.length} steps).`);
  }
  console.log('Done. The house recipes are live in mothermode_recipes.');
}
