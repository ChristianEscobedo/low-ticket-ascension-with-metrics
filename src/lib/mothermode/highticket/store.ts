/**
 * High Ticket Kit store. Admin-only tool: every read and write uses the
...
const TABLE = 'mothermode_high_ticket_kits';

const COLUMNS =
  'id, slug, name, status, intake, kit, context_refs, created_at, updated_at, updated_by';
...
export async function listKitsForAdmin(): Promise<HighTicketKitRecord[]> {
  try {
    ...
    return (data as HighTicketKitRow[]).map(rowToHighTicketKit);
  } catch {
    return [];
  }
}
...
export async function getKitById(id: string): Promise<HighTicketKitRecord | null> {
  try {
    const { data, error } = await (serviceClient() as any)
      .from(TABLE)
      .select(COLUMNS)
    ...
    return rowToHighTicketKit(data as HighTicketKitRow);
  } catch {
    return null;
  }
}
...
export async function getKitBySlug(slug: string): Promise<HighTicketKitRecord | null> {
  try {
    const { data, error } = await (serviceClient() as any)
      .from(TABLE)
      .select(COLUMNS)
...
export async function upsertKit(input: UpsertKitInput): Promise<HighTicketKitRecord> {
