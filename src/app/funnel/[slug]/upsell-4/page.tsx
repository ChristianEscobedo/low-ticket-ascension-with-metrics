...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  try {
    ...
    return {
      title: `${funnel.upsell4.headline || 'Special Offer'} | MotherMode`,
      description: funnel.upsell4.subheadline || undefined,
    };
  } catch {
...
/**
 * OTO4 — editable funnel upsell driven by funnel.upsell4 JSON.
 * Renders the exact MotherMode OTO layout with admin hover-to-edit.
 */
export default async function FunnelUpsell4Route({ params, searchParams }: Props) {
  const { funnel, isAdmin, gated } = await loadSalesFunnelPage(params.slug, undefined, {
    pp: searchParams?.pp,
  });
  if (gated) return <GatedPage />;
  return <UpsellPage funnel={funnel} upsellKey="upsell4" isAdmin={isAdmin} />;
}
