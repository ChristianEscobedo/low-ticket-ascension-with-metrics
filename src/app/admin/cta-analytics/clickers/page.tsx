...
export default async function CtaClickersPage({
  ...
  return (
    <div>
        ...
        <div>
          ...
          {context?.ctaLink && (
            <a
              ...
              className="inline-flex items-center gap-1 text-xs text-bone/50 hover:text-brass mt-2"
            >
              {context.ctaLink}
      ...
      <div className="mt-8 rounded-2xl border border-brass/15 bg-gradient-to-br from-mode-deep/40 to-ink/70 backdrop-blur overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-bone/[0.03] text-[11px] uppercase tracking-wider text-brass/80">
            <tr>
          ...
          <tbody>
            {clickers.length === 0 && (
              <tr>
                <td
                  colSpan={5}
                  className="px-4 py-10 text-center text-bone/40"
                >
                  <Users className="w-6 h-6 mx-auto mb-2 text-brass/30" />
            ...
            {clickers.map((c) => (
              <tr
                ...
                className="border-t border-bone/5 hover:bg-bone/[0.02]"
              >
                <td className="px-4 py-2.5">
                  {c.user_id ? (
                    ...
                    <span className="text-bone/30 italic">Anonymous</span>
                  )}
                </td>
                <td className="px-4 py-2.5 text-right">
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-brass/[0.08] border border-brass/30 text-brass text-xs font-semibold">
                ...
                <td className="px-4 py-2.5 text-right">
                  {c.user_id ? (
                    <Link
                      ...
                      className="text-brass hover:text-brass/80 text-xs"
                    >
                      open ↗
                    </Link>
                  ) : (
