...
import {
  ...
  upsertKit,
  deleteKit,
} from '@/lib/mothermode/community/store';
import {
  normalizeIntake,
  normalizeKit,
  toCommunityType,
...
/**
 * GET: admin-only. Lists every community kit (drafts included) for the
 * /admin/community editor. Returns { success, admin, items }.
 */
...
/**
 * POST: admin-only. Create or update one community kit.
 * Body: { id?, slug, name, communityType, status, intake, kit }
 */
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { id, slug, name, communityType, status, intake, kit, contextRefs } =
      body ?? {};
    ...
    const saved = await upsertKit({
      ...
      communityType: toCommunityType(communityType),
      status: toCommunityStatus(status),
...
/**
 * DELETE: admin-only. Remove one community kit by id. Query: ?id=uuid
 */
