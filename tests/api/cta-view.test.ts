...
describe('POST /api/courses/cta-view', () => {
  it('returns 400 when lesson_id is missing', async () => {
    const { POST } = await import('@/app/api/courses/cta-view/route');
  ...
  it('silently skips author-time preview views without inserting', async () => {
    const { POST } = await import('@/app/api/courses/cta-view/route');
