...
## The production arc (2026-08-08 → 08-09, changelog 2.26.0 → 2.33.0)

- **The steer (2.26.0):** hookFamilies.ts — the registry of ten hook families (template + example + the visual hook + the awareness gate). The Producer intake's 80% + steer block (awareness, hook family, CTA target, prompt-bank framework) rides the script call AND the planner call (a "PRODUCER'S STEER" block in the grounding — scenes + sheet groupings form around it). The run card's "generate hooks" writes five hooks across the legal families; "use" swaps scene 1's line (the gate re-opens).
- **The product (2.27.0 + 416eb32):** ClonePlan.productImageUrl — the thing being sold rides the manifest: the sheet prompts stage it in the panels, every b-roll render carries it as a reference (after the character, before the world sheet), and every sheet FORGE sends it in the reference array.
- **The voice pass + the cast strip (2.28.0):** the intake twin picker is a card strip; "+ new session" clears everything; the run card's voice pass voices every spoken beat audio-only first (a beat with audio never re-buys).
- **Sheet edits + the model select (2.29.0):** per-sheet edit notes (re-forge seeded with itself) + regen + note (re-forge from scratch, the adjustment winning); burn-the-script toggle (each panel wears its timestamp + line — defaults ON, a real switch); per-scene Seedance model select (2.0 omni vip default / 2.5 hero) saved per beat. The ElevenLabs key falls back to /admin/integrations (provider elevenlabs).
- **The production brief (2.30.0 + 2.30.1):** cloneBrollPrompt + cloneAvatarPrompt are production documents — the [REFERENCE SYSTEM] header tags each ref by role (@image1 THE CHARACTER, the variant, the PRODUCT, the WORLD sheet — numbered in the route's exact ref order), the [SOURCE OF TRUTH PRIORITY] table resolves conflicts by category, and the beat block runs SCENE → BEAT → ACTION → SCRIPT → DELIVERY → CUT → CONTINUITY with the timeline window inline. The Producer's on-screen prompt is the identical brief the route sends.
- **The voice sample (2.32.0):** ClonePlan.voiceSampleUrl — paste or upload (audio or a video of them talking) on the intake OR the run card any time. The route's voice leg checks it FIRST: the beat stamps voiced with the sample as the audio reference — zero ElevenLabs calls. Per-character samples: ReelClone.voiceSampleUrl overrides the plan's.
- **Extend + the cast (2.33.0):** the run card's "extend the production" block — + add scene (kind, the line/visual, which world sheet it quotes, or "+ a new world" forges one on the spot; the gate re-opens) and + world sheet (a new environment, scenes join it later). Multi-character: ClonePlan.characters[] (the cast — full twins), CloneBeat.characterId, cloneBeatCharacter() + cloneBeatVoiceSample() resolvers; the route resolves WHO per beat (refs + prompt + voice all follow). The cast row adds a roster twin; every scene card picks who's in it.
